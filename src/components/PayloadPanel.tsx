...
/**
 * Register custom SWIFT MT language for Monaco editor
 */
function registerSwiftMTLanguage(monaco: Monaco) {
  if (monaco.languages.getLanguages().some((lang: languages.ILanguageExtensionPoint) => lang.id === 'swift-mt')) {
    return;
  }
  ...
  monaco.editor.defineTheme('swift-mt-light', {
    base: 'vs',
    inherit: true,
    rules: [
  ...
  monaco.editor.defineTheme('swift-mt-dark', {
    base: 'vs-dark',
    inherit: true,
    rules: [
...
/**
 * Detect the input format based on content
 */
function detectInputFormat(text: string): 'xml' | 'json' | 'swift-mt' {
...
export function PayloadPanel({ payloadText, onPayloadChange, isDark }: PayloadPanelProps) {
  const inputFormat = useMemo(() => detectInputFormat(payloadText), [payloadText]);
  ...
  const getEditorConfig = useCallback(
    (format: 'xml' | 'json' | 'swift-mt') => {
      if (format === 'swift-mt') {
        return {
          language: 'swift-mt',
          theme: isDark ? 'swift-mt-dark' : 'swift-mt-light',
        };
      }
      return {
        language: format,
  ...
  const editorConfig = useMemo(() => getEditorConfig(inputFormat), [getEditorConfig, inputFormat]);
    ...
    <div className="h-full flex flex-col bg-white dark:bg-gray-900 border-l border-gray-200 dark:border-gray-700">
      ...
      <div className="px-3 py-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex items-center justify-between">
        ...
        <span className="text-xs text-gray-500 dark:text-gray-400">
          {inputFormat === 'swift-mt' ? 'SWIFT MT' : inputFormat.toUpperCase()}
        </span>
      </div>
